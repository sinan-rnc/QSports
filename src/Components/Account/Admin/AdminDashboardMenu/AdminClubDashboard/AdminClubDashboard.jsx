...
import "./AdminClubDashboard.scss"
...
export default function AdminClubDashboard() {
    ...
    const [ featuredClubID, setFeaturedClubID ] = useState(false)
    const [ openEditClubSection, setOpenEditClubSection ] = useState(false)
    ...
    useEffect(() => {
        if (currentClubID) {
            const club = allClubs.find(ele => ele._id === currentClubID);
            if (club) {
                setCurrentClub(club);
            }
        }
    ...
    useEffect(() => {
        if(currentClub) {
            setForm({
                ...
                contactPerson: currentClub.contactPerson,
                clubType: currentClub.clubType,
                city: currentClub.city,
                slogan: currentClub.slogan,
                image: currentClub.image,
                emailAddress: currentClub.emailAddress,
                phoneNo: currentClub.phoneNo,
                webSite: currentClub.webSite,
                experience: currentClub.experience,
                address: currentClub.address,
                latitude: currentClub.geoLocation?.coordinates[1],
                longitude: currentClub.geoLocation?.coordinates[0],
                introductionObjtv: currentClub.introductionObjtv,
                openTime: convertTo24HourFormat(currentClub.openTime),
                closeTime: convertTo24HourFormat(currentClub.closeTime),
                happyHrRates: currentClub.happyHrRates,
                normalHrRates: currentClub.normalHrRates,
                description: currentClub.description,
                history: currentClub.history,
                youtubevideo: currentClub.youtubevideo,
                pictureGallery: currentClub.pictureGallery,
                socialMedialinks: currentClub.socialMedialinks,
                services: currentClub.services,
            })
        } else {
            setOpenEditClubSection(false)
        }
    }, [currentClub])

    useEffect(() => {
        if (featuredClubID) {
            const club = allClubs.find(ele => ele._id === featuredClubID);
            if (club) {
                setCurrentClub(club); // Update currentClub
                setFormData({
                    _id: club._id,
    ...
    const clubServices = currentClub?.services?.filter(service => availableServices.some(ele => service.name.includes(ele)))
    .map(service => service.name);

    // console.log(clubServices)

    const clubFoodServices = currentClub?.services?.filter(service => availableFoodServices.some(ele => service.name.includes(ele)))
    .map(service => service.name);
    ...
    const [selectedServices, setSelectedServices] = useState(currentClub?.services ? clubServices : form.services);
    const [selectedFoodServices, setSelectedFoodServices] = useState(currentClub?.services ? clubFoodServices : form.services);
    ...
    const handleEditClub = async (clubId) => {
        ...
        setOpenEditClubSection(true)
    }

    const handleFeatureClub = async (ClubID) => {
        setFeaturedClubID(ClubID)
        // console.log(featuredClubID)
    }
    ...
    const handleFormSubmit = async (e) => {
        ...
        if(Object.keys(errors).length === 0) {
            if (currentClub) {
                formData.append("_id", currentClub._id);
                // console.log("for update, formData", form);
                // const updatedForm = { ...form, _id: currentClub._id }
        ...
        <section className="admin-club-dashboard-container">
            ...
            {openEditClubSection &&
                <section id="edit-club" className="edit-club-section">
                    <div className="dashborad-heading" onClick={() => {setOpenEditClubSection(!openEditClubSection)}}>
                        <h1 className='dashborad-main-heading'>Update Club</h1>
                        <hr className={`dashborad-hr-1 ${openEditClubSection && "rotate"}`}/><hr className="dashborad-hr-2"/>
                        <h3 className="dashborad-second-heading">{currentClub && currentClub.name}</h3>
                    </div>
                    <form className="form-table" onSubmit={handleFormSubmit}>
                        ...
                        <div className="btn-div">
                            <button className="save-btn" type="submit">{currentClub ? "Save" : "Register"}</button>
